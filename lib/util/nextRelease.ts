...
export async function nextRelease(ctx: Context, tagPrefix?: string, defaultVersion?: string) {
  ...
  const version = ctx.preReleaseTag != null
    ? `${coerced?.version}-${ctx.preReleaseTag}.0`
    : coerced?.version
  ...
  ctx.logger
    .warn(`No previous release found. Next release will default to your package.json version ${ctx.nextRelease.version}`)
}
